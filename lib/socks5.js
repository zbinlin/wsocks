    ...
    var server = net.createServer({
        ...
        socket.on("readable", function __readable_handle__() {
            ...
            remoteSocket.once("connect", function () {
                ...
                var buf = new Buffer([0x05, 0x00, 0x00]);
                var host = socket.address();
                var addressType = new Buffer([0x00]);
                if (host.family === "IPv4") {
                    addressType[0] = 0x01;
            ...
            function onError(err) {
                if (err.errno === "ECONNREFUSED") {
                    socket.end(new Buffer([0x05, 0x05, 0x00]));
                } else {
                    socket.end(new Buffer([0x05, 0x04, 0x00]));
                }
            }

            function destroy() {
                socket.removeListener("readable", __readable_handle__);
                socket.end(new Buffer([0x05, 0x01, 0x00]));
            }
